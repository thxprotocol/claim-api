...
async function fetchBalance(chainId: number, address: string) {
    ...
    const bal = await axios
        .get(`https://api.covalenthq.com/v1/${chainId}/address/${address}/balances_v2/?key=${COV_PRIVATE_KEY}`);

    // Filter the result to ignore any unnecessary tokens.
    return bal.data.data.items.filter((item: { [k: string]: any }) => validTokens.includes(item.contract_ticker_symbol));
}
